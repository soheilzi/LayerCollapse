...
class PositionWiseFeedForwardLeaky(nn.Module):
    """ FeedForward Neural Networks for each position """
    def __init__(self, cfg):
        super().__init__()
        self.fc1 = nn.Linear(cfg.dim, cfg.dim_ff)
...
class BlockLeaky(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.proj = nn.Linear(cfg.dim, cfg.dim)
        self.norm1 = LayerNorm(cfg)
        self.pwff = PositionWiseFeedForwardLeaky(cfg)
        self.norm2 = LayerNorm(cfg)
        self.drop = nn.Dropout(cfg.p_drop_hidden)
...
class BlockIdentity(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.proj = nn.Linear(cfg.dim, cfg.dim)
        self.norm1 = LayerNorm(cfg)
        self.pwff = PositionWiseFeedForwardIdentity(cfg)
        self.norm2 = LayerNorm(cfg)
        self.drop = nn.Dropout(cfg.p_drop_hidden)
...
class Transformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.transformer_layers = [Block(cfg) for _ in range(cfg.n_layers-8)]
        self.transformer_layers.append(BlockLeaky(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.transformer_layers.append(BlockIdentity(cfg))
        self.blocks = nn.ModuleList(self.transformer_layers)
